...
async def wait_for_breakout(exchange, settings: BotSettings, plan: ArmedPlan) -> Optional[str]:
    ...
    symbol = f"{settings.symbol}:USDT" if not settings.symbol.endswith(":USDT") else settings.symbol
    ...
    try:
        while not engine_stop.is_set():
            ...
            if direction == "long":
                ...
                if r >= settings.trailing_start_r:
            ...
            if hit_stop or hit_tp2 or stale_exit or timeout_exit:
                reason = "stop" if hit_stop else "tp2" if hit_tp2 else "stale_exit" if stale_exit else "timeout"
                if remaining > 0:
                    await close_position_market(exchange, settings.symbol, side_close, remaining, direction if settings.hedge_mode else None)
                ...
                db.update_trade_by_client_oid(active_oid, {
                    ...
                    "close_price": price,
                    "close_reason": reason,
                    "meta": {"entry": entry, "last": price, "gross_pnl": realized, "fee_est": fee_est, "tp1_done": tp1_done, "score": plan.metrics.get("volatility_score")},
                })
...
async def maybe_auto_arm_volatility(settings: BotSettings) -> bool:
    ...
    db.set_runtime_state(BotMode.VOLATILITY_SCAN.value, True, f"vol score {score:.1f} / {state}")

    if score >= settings.notify_score:
        if not _last_hot_log_at or (utc_now() - _last_hot_log_at).total_seconds() > 60:
            db.log_event("info", "volatility_watch", f"Market volatility score {score:.1f}: {state}", market)
    ...
    if score >= settings.auto_arm_score and market.get("valid_for_sniper"):
        ...
        event = synthetic_event(
            ...
            f"AUTO VOLATILITY HUNT score={score:.1f}",
            raw={"market": market},
        )
...
async def start_engine() -> Dict[str, Any]:
    ...
    return {"status": "started", "mode": "hybrid_scan", "calendar_enabled": settings.calendar_enabled, "volatility_auto_enabled": settings.volatility_auto_enabled}
